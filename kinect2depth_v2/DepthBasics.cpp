...
void CDepthBasics::ProcessDepth(INT64 nTime, UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
    ...
    if (m_hWnd)
    {
		...
		nx = clamp( nWidth - 1 - nx, 0, nWidth  - 1 );
		ny = clamp( ny, 0, nHeight - 1 );

		unsigned intensity = pBuffer[ nx + ny * nWidth ];
    ...
    if (m_pDepthRGBX && pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
    {
			...
			{
				RGBQUAD color;
				intensity_to_rgb( s_buffer[ i ], nMinDepth, nMaxDepth, color.rgbRed, color.rgbGreen, color.rgbBlue );
